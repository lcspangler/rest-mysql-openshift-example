...
public class StopPaymentIntegration {
	...
	private TestMySqlStopPaymentRepository stopPaymentRepository = new TestMySqlStopPaymentRepository();

	@Before
	public void setup() {
		// Clear test data using test impl of the repository, before each test
		stopPaymentRepository.clearAllStopPayments();
	}
	...
	public void shouldSaveAndRetrieveStopPayment() {
		...
		stopPaymentRepository.addStopPayment(stopPaymentForSave);

		// Retrieve and check
		List<StopPayment> retrievedStopPayments = stopPaymentRepository.getAllStopPayments();
